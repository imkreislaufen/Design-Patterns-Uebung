...
public class Zaehlsteuerung extends JFrame {
	...
	public static Zaehlsteuerung getSteuerung(final Display[] listDisp, int unterklasse) {
		if (einzigeSteuerung == null) {
			if (unterklasse == 0) {
				einzigeSteuerung = new TischtennisZaehler(listDisp);
			} else if (unterklasse == 1) {
				einzigeSteuerung = new BasketballZaehler(listDisp);
			} else {
				throw new IllegalArgumentException("Bisher sind nur 0 für Tischtennis und 1 für Basketball"
						+ " auswählbar, weitere Sportartenzähler werden aber" + "noch folgen!");
			}
		}
	...
	protected JButton punktHeim;
	protected JButton punktGast;
	protected JButton loeschen;

	// Punktestände definieren
	protected int punktestandHeim;
	protected int punktestandGast;
